...
if (_art == "init") then {
	...
	INV_BombDefuse = {
		private ["_id", "_vcl", "_name", "_status"];
		_id = _this select 0;
		_vcl = _this select 1;
	...
	if (isServer) exitWith {["server"] execVM "bombs.sqf";};
};
	...
	while {true} do {
		...
		for [{_i=0}, {_i < (count INV_ServerBombArray)}, {_i=_i+1}] do {
			...
			if (isNull(_vehicle)) exitWith {
				format["""%1"" call INV_BombDelete", _id] call ISSE_pub_execPstr;
				_skipWait = true;
			};
			...
			switch (_name) do {
				case "fernzuenderbombe": {
					...
					if (_status == "defused") then { format["""%1"" call INV_BombDelete", _id] call ISSE_pub_execPstr;};
				};
				...
				case "nuke": {
					_vanished = false;
					if (_settings select 0 < time) then { [{ (_this select 0) spawn SLR_fnc_nuke; }, [_vehicle]] call SLR_fnet_execPublic; _vanished = true; };
					if (_status == "defused") then { format["""%1"" call INV_BombDelete;", _id] call ISSE_pub_execPstr; _vanished = true; };

					if (_vanished) then {
						[{ [] call SLR_fnc_nuclearCleanup; }, []] call SLR_fnet_execPublic;
					};
				};
				...
				case "geschwindigkeitsbombe": {
					...
					if ( (!_active) and (_speed > (_settings select 0)) ) then {
						...
						format["if (player in %1) then {player groupChat localize ""STRS_inv_item_vehiclebomb_speedbomb_activated"";};", _vehicle, (_settings select 0), (_settings select 2)] call ISSE_pub_execPstr;
					};
					if ( ((_settings select 3) != -1) and (time   > (_settings select 4)) ) then {
						_array set [3, "defused"];
						format["if (player in %1) then {player groupChat localize ""STRS_inv_item_vehiclebomb_deactivated"";};", _vehicle] call ISSE_pub_execPstr;
					};
				};
			};
		};
	...
	while {true} do {
		...
		for [{_i=0}, {_i < (count INV_LocalBombArray)}, {_i=_i+1}] do {
			...
			if ( (vehicle player == _vehicle) and ((count _actionArr) == 0) ) then {
				...
				switch _item do {
					case "fernzuenderbombe": {
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate",  "someCompile.sqf", format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];
						_actionArr = _actionArr + [_a1];
					};
					case "zeitzuenderbombe": {
						_a1 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_deactivate", "someCompile.sqf",  format["[""config"", ""defuse"", ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];
						_a2 = _v addAction [localize "STRS_inv_item_vehiclebomb_action_changetimer", "someCompile.sqf", format["[""config"", ""Timer"",  ""%1"", ""%2""] execVM ""bombs.sqf"";", _item, _id ]];
			...
			if (vehicle player != _vehicle) then {
				for [{_j=0}, {_j < (count _actionArr)}, {_j=_j+1}] do {
					_v removeAction (_actionArr select _j);
				};
				_actionArr = [];
			};
...
if (_art == "use") then {
	...
	if ((_item == "zeitzuenderbombe") || (_item == "nuke")) then {
		if (!(createDialog "timebombconfig")) exitWith {hint "Dialog Error!";};
		if (_item == "nuke") then {
			sliderSetRange [1, 1200, 3200];
		} else {
			sliderSetRange [1, zeitbombe_mintime, zeitbombe_maxtime];
		};
		sliderSetSpeed [1, 1, 10];
		buttonSetAction [4, format["[""plant"", ""%1"", sliderPosition 1] execVM ""bombs.sqf""; closedialog 0;", _item]];
	...
	if (_item == "aktivierungsbombe") then {["plant", "aktivierungsbombe"] execVM "bombs.sqf";};
	...
	if (_item == "fernzuenderbombe") then {["plant", "fernzuenderbombe"] execVM "bombs.sqf";};
	if (_item == "fernzuender") then {if (!(createDialog "remotebomb_remote")) exitWith {hint "Dialog Error!";};};
};
...
if (_art == "plant") then {
	...
	format ["if (player in %1) then {player groupChat format[localize ""STRS_inv_item_vehiclebomb_legen_gesehen"", %2];};", _vcl, player] call ISSE_pub_execPstr;
	...
	switch _item do {
		...
		case "nuke": {
			...
			[{ _this call SLR_fnc_nuclearWarning; }, [_vcl, (_this select 2)]] call SLR_fnet_execPublic;
	...
	( format[ "[%1, ""%2"", %3, ""%4"", ""%5"", %6] call INV_BombAdd;", player, _id, _vcl, _item, "planted", _settings] ) call ISSE_pub_execPstr;
	...
	switch _name do {
		case "fernzuenderbombe": {
			...
			if (_art == "activate") then {
				format["if (isServer) then {[""%1"", [%2]] call INV_BombEdit;};", _id, true] call ISSE_pub_execPstr;
			};
		};
		case "zeitzuenderbombe": {
			if (_art == "defuse") then {
				format["""%1"" call INV_BombDelete", _id] call ISSE_pub_execPstr;
				[_name, 1] call INV_AddInventoryItem;player groupChat localize "STRS_inv_item_vehiclebomb_defused";
			};
			if (_art == "timer") exitWith {
				...
				if (zeitbombencounterchanged == 1) then {
					format["if (isServer) then {[""%1"", [%2]] call INV_BombEdit;};", _id, (time+_sliderpos)] call ISSE_pub_execPstr;
		...
		case "nuke": {
			...
			if (_art == "timer") exitWith {
				...
				_sliderpos = 0;
				ctrlSetText [4, localize "STRS_inv_item_vehiclebomb_changetimer"];
				sliderSetRange [1, 1200, 3200];sliderSetSpeed [1, 1, 10];
				buttonSetAction [4, "zeitbombencounterchanged = 1; closedialog 0;"];
				zeitbombencounterchanged = 0;
				while {ctrlVisible 2} do {
					_sliderpos = round(sliderPosition 1);
