//Re-typing of variable types
// Most of this could be replaced by a single command (parseNumber for example), but too much stuff to change - keeping it as it is now

ISSE_MAX_NUMBER = 2^30;
					...
					private["_arr", "_i", "_num", "_isNumber", "_KommataCount", "_result", "_reString"];	
					   ...
					   if ((typeName _this) == "STRING") then {
							...
							for [{_i=0}, {_i < (count _arr)}, {_i=_i+1}] do {
								...
								if (    ( (!(_num in ISSE_str_const_StringNum)) and ((_num != "-") and (_i == 0)) ) or (_KommataCount > 1)    ) exitWith {_isNumber = false;};
							};
						...
						ISSE_str_isInteger = { 
							private["_num"];
							_num = _this call ISSE_str_GetInteger;(  (_num select 0) and ((_num select 1) == (round (_num select 1)))  )
						};
						ISSE_str_isFloat = {
							((_this call ISSE_str_GetInteger) select 0)
						};
						ISSE_str_StrToInt = {
							(parseNumber _this)
						};
						ISSE_str_StrToLongint = {
							( round ((_this call ISSE_str_GetInteger) select 1) )
						};
						ISSE_str_StrToFloat = {
							((_this call ISSE_str_GetInteger) select 1)
						};
						ISSE_str_IntToStr = {
							(str _this)
						};
						...
						ISSE_str_Length = {
							if ((typeName _this) == "STRING") then {
								(count (_this call isse_str_StrtoArr))									
							} else {
							-1						
							};
						};
						...
						ISSE_str_IsInStr = {
							( ([(_this select 0), (_this select 1)] call ISSE_str_Pos) > -1 )
						};
						ISSE_str_Pos = {
							...
							for [{_i=0}, {_i < (((count _str)-(count _search))+1)}, {_i=_i+1}] do {	
								...
								for [{_j=0}, {_j < (count _search)}, {_j=_j+1}] do {
									if (not ((_str select (_i+_j)) == (_search select _j)) ) exitWith {_found = false;};
								};
