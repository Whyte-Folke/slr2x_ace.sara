...
if (_art == "activate") then
{
   ...
   if (_turnOn) then
   {
      format ["[0,0,0,[""client"", %1]] execVM ""carSiren.sqf"";", player] call ISSE_pub_execPstr;
   }
   else
   {
...
if (_art == "client") then
{
   ...
   _lichtscript = [0,1,2,["licht", _vcl, [_light1, _light2]]] execVM "carSiren.sqf";
   ...
   _vcl setVariable ["rpp_strobeson",false,false];
};
