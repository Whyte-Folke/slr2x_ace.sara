 ...
 { 
 if (not(isnull _x)) then { _index = lbAdd [1, format ["%1 (%2)", typeOf _x, _x]]; lbSetData [1, _index, format ["%1", _x]]; };	
 } forEach INV_VehicleArray;																																	
 
 buttonSetAction [2, "if ((lbCurSel 1) > -1) then { [""schluessel"",          (INV_VehicleArray select lbCurSel 1), lbCurSel 1] execVM ""carKeys.sqf"";}; closedialog 0;"];								
 buttonSetAction [4, "if ((lbCurSel 1) > -1) then { [""dropschluessel"",      (INV_VehicleArray select lbCurSel 1), lbCurSel 1] execVM ""carKeys.sqf"";}; closedialog 0;"];											
 buttonSetAction [5, "if ((lbCurSel 1) > -1) then { [""schluesseluebergabe"", (INV_VehicleArray select lbCurSel 1), lbCurSel 1] execVM ""carKeys.sqf"";}; closedialog 0;"];};
 ...
 {																																													
 format ["%1 lock true;", _vcl] call ISSE_pub_execPstr;															
 ...
 {																																													
  ...
  format ["%1 lock false;", _vcl] call ISSE_pub_execPstr;															
  ...
  format ["if (INV_ROLESTRING == ""%1"") then {INV_VehicleArray = INV_VehicleArray + [%2]; player groupChat localize ""STRS_inv_items_uebergabe_schluessel_success"";};", _spieler, _vcl] call ISSE_pub_execPstr;		
  INV_VehicleArray = INV_VehicleArray - [_vcl];	
